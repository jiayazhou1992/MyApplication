...
public class RefreshLayout extends ViewGroup implements NestedScrollingParent {
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (scrollerCompat.computeScrollOffset()||doing)return;
        //Log.e(TAG,"dy=="+dy);
        if (dy<0){
            if (!canChildScrollUp()){
                if (refreshHead.getFixType()==0&&getScrollY()>=-refreshHead.getHight()){
                    if (getScrollY()+dy2<=-refreshHead.getHight()){
                        dy2=-getScrollY()-refreshHead.getHight();
                    }
                    scrollBy(0,dy2);
                    consumed[1]=dy;
                }
    ...
    public void onStopNestedScroll(View child) {
        ...
        else if (refreshFoot.getFixType()==1){
        ...
        if (refreshHead.getFixType()==0){
            if (getScrollY()<=-refreshHead.getHight()){
                Log.i(TAG,"head-----------------1");
                scrollBack(-refreshHead.getHight()-getScrollY());
                if (onRefreshAndLoadingListener!=null){
        ...
        else if (refreshHead.getFixType()==1){
